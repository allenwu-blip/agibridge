...
class HealthResponse(BaseModel):
    """GET /api/v1/health. Spec §2.4 v2.2 + D4-harden Check 3b.

    PUBLIC pre-auth endpoint (Clerk-exempt) once the HF Space flips public.
    Trimmed to the minimal liveness contract: `free_disk_mb` (infra
    capacity), `uptime_s`, and `active_session` are all info-leak / fingerprint
    surface on an unauthenticated endpoint and are dropped. `version` is the
    embodied-data version (renamed from `embodied_data_version`). Deeper
    build/version detail stays on the deliberate `/version` endpoint."""
